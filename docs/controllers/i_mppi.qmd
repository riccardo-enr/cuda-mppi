...
To verify the effectiveness of the informative reward, we conducted a simulation campaign in a "corridor with a hole" scenario. The robot starts at (1, 5) and is tasked with moving forward. 

![I-MPPI Exploration Campaign. The heatmap (Plasma) shows the Information Gain field. Cyan lines indicate the true wall locations. The Standard MPPI (black dashed) remains in the safe corridor, while the Informative MPPI (white solid) proactively explores the high-entropy region through the gap.](../_media/imppi_campaign.png)
