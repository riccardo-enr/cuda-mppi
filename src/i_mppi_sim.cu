...
struct SimpleDynamics
{
  ...
  void step_host(
    Eigen::VectorXf & state, const Eigen::VectorXf & u,
    float dt) const
  {
    state[0] += state[2] * dt;
    state[1] += state[3] * dt;
...
void save_info_gain_map(
  FSMICost & cost, int width, int height, float resolution,
...
void update_map_observations(
  std::vector<float> & h_map, float x, float y,
...
float calculate_area_entropy(
  const std::vector<float> & map, int width, int x_min_idx,
  int x_max_idx, int y_min_idx, int y_max_idx)
{
  float entropy = 0.0f;
