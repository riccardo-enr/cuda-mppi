...
struct QuadrotorDynamics {
    ...
    __device__ void step(const float* x, const float* u_raw,
        ...
        // k2
        for (int i = 0; i < 13; ++i) tmp[i] = x[i] + 0.5f * dt * k1[i];
        state_deriv(tmp, u, k2);

        // k3
        for (int i = 0; i < 13; ++i) tmp[i] = x[i] + 0.5f * dt * k2[i];
        state_deriv(tmp, u, k3);

        // k4
        for (int i = 0; i < 13; ++i) tmp[i] = x[i] + dt * k3[i];
    ...
    void step_host(Eigen::VectorXf& state, const Eigen::VectorXf& action, float dt) const {
        ...
        for (int i = 0; i < 13; ++i) x[i] = state(i);
        for (int i = 0; i < 4; ++i) u_raw[i] = action(i);
        ...
        for (int i = 6; i < 10; ++i) x_next[i] /= qn;

        for (int i = 0; i < 13; ++i) state(i) = x_next[i];
    }
};
